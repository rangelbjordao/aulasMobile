...
// Definir como o app vai reagir quando receber uma notificacao
Notifications.setNotificationHandler({
    // Funcao assincrona chamada automaticamente ao receber uma notificacao
    handleNotification: async () => ({
        // Define se a notificacao vai aparecer como alerta visual(popup/banner)
        // quando true ele exibe a notificacao mesmo com o app aberto
        shouldShowAlert: true,
        shouldPlaySound: true, // toca som de notificacao
...
export default function Home() {
    ...
    // Estado para armazenar o token de notificacao
    const [expoPushToken, setExpoPushToken] = useState<string | null>(null)

    const router = useRouter(); //Hook de navegação

    /*Executa em tempo real a coleção de produtos será usuário logado
    Sempre que algo muda no Firestore, a lista é atualizada automaticamente na tela*/
    useEffect(() => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (finalStatus !== "granted") {
                alert("Permissao de notificacao nao foi concedida.")
                return
            }
        })()
    }, [])
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                console.log("Error ao gerar token de notificacao", error)
            }
        })
    })
    ...
    // Disparar notificacao local
    const dispararNotificacao = async () => {
        await Notifications.scheduleNotificationAsync({
            content: {
                title: "Notificacao Teste",
                body: "Aula de notificacoes push",
            },
            trigger: null, // Ela vai ser disparada sem agendamento(na hora)
        });
    };

    // Criando funcao para pegar o token de notificacao do usuario
    const registerPushNotificationAsync = async (): Promise<String | null> => {
        try {
            const tokenData = await Notifications.getExpoPushTokenAsync()
            const token = tokenData.data
            console.log("Token de notificacao gerado com sucesso:", token)
            return token
        } catch (error) {
            console.log("Erro ao gerar token de notificacao", error)
            return null
        }
    }
    ...
    return (
        <SafeAreaView style={styles.main}>
            ...
            >
                ...
                />
                <Button title="Disparar Notificacao Local" color="purple" />
